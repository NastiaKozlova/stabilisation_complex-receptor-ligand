...
df_tcl<-read.csv(paste0("make_picture_tcl_surf/",v_structure[1],".tcl"),stringsAsFactors = F)
i<-2
for (i in 2:length(v_structure)) {
  df_tcl_add<-read.csv(paste0("make_picture_tcl_surf/",v_structure[i],".tcl"),stringsAsFactors = F)
