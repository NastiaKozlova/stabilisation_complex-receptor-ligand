...
df_merge<-read.csv(paste0(part_analysis,"din/log_merge.csv"),stringsAsFactors = F)
...
i<-1
if(dir.exists(paste0(part_analysis,"din/complex_structure"))) {system(command = paste0("rm -r ",part_analysis,"din/complex_structure"),ignore.stdout=T,wait = T)}

if(!dir.exists(paste0(part_analysis,"din/complex_structure"))){dir.create(paste0(part_analysis,"din/complex_structure"))}
for (i in 1:nrow(df_merge)) {
  ...
  write.pdb(pdb_complex,paste0(part_analysis,"din/complex_structure/",df_merge$name.x[i]))
}
