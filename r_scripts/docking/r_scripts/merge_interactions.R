...
if (dir.exists(paste0("interaction_merge/"))) {system(command = paste0("rm -r ",part_name,"din/interaction_merge/"))}

if (!dir.exists(paste0("interaction_merge/"))) { dir.create(paste0("interaction_merge/"))}
i<-1
j<-3
p<-1
...
for (j in 1:length(v_structure)) {
  ...
  if(nrow(df_pdb)==test){
    ...
    write.csv(df_pdb,
              paste0("interaction_merge/",v_structure[j],".csv"),row.names = F)
  }
}
...
df_pdb<-read.csv(paste0("interaction_merge/",df_all$name.x[1],".csv"),stringsAsFactors = F)
df_pdb<-df_pdb%>%mutate(name.x=df_all$name.x[1])
df_pdb<-df_pdb%>%filter(persent_interactions==100)
for (j in 2:nrow(df_all)) {
  df_pdb_add<-read.csv(paste0("interaction_merge/",df_all$name.x[j],".csv"),stringsAsFactors = F)
...
#  geom_rect(aes(xmin = seq_beg-0.5, xmax = seq_end+0.5, ymin = -Inf, ymax = Inf,fill=topology,alpha=0.1),data=df_topology)+
  ...
  guides(alpha = "none")
ggsave(p,   filename = paste0("merge_interactions.png"), width = 60, height = 30, units = c("cm"), dpi = 200 ) 