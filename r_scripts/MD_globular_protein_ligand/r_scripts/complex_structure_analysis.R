...
for (name in 1:length(based_name)) {
  ...
  write.csv(df_fin,paste0("din/intercations.csv"),row.names = F)
  v_break<-unique(seq(from=round(min(df_fin$Total),digits = -1),to=round(max(df_fin$Total),digits = -1),by=20))
  df_fin<-df_fin%>%filter(Total<0)
  p<-ggplot(data = df_fin)+
    geom_freqpoly(aes(x=Total),binwidth=10)+theme_bw()+
  ...
  df_fin<-read.csv(paste0("din/intercations.csv"),stringsAsFactors = F)
  df_fin<-df_fin%>%mutate(number_interactions=0)
  for (i in 1:nrow(df_fin)) {
    ...
    y<-1
    if(length(m)>0){
      df_fin$number_interactions[i]<-length(m)
      for (y in 1:length(m)) {
  ...
  write.csv(df_fin,paste0("din/intercations.csv"),row.names = F)
  df_fin<-df_fin%>%filter(number_interactions!=0)
  df_intecations<-pdb$atom
  df_intecations<-df_intecations%>%mutate(interactions=0)
  df_intecations<-df_intecations%>%filter(elety=="CA")
  i<-1
  for (i in 1:nrow(df_fin)) {
    df_intecations_TEMP<-read.csv(paste0("din/interaction/",based_name[name],"/frame_",df_fin$number[i],".csv"),stringsAsFactors = F)
